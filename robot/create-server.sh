...
aws ec2 run-instances  --image-id $AMI_ID  --instance-type t2.micro --security-group-ids $SGID  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=$COMPONENT}]" 'ResourceType=volume,Tags=[{Key=cost-center,Value=cc123}]'| jq 