...
echo show databases | mysql -uroot -p${SQL_PASSWORD} &>> $LOGFILE 
if [ $? -ne 0 ] ; then 
    echo -n "Reset Root Password"
    echo "ALTER USER 'root'@'localhost' IDENTIFIED BY '${SQL_PASSWORD}';" | mysql  --connect-expired-password  -uroot -p"${DEF_ROOT_PASSWORD}" &>> $LOGFILE 
    stat $? 
fi 
...
if [ $? -eq 0 ] ; then 
    ...
    echo "uninstall plugin validate_password;" | mysql  --connect-expired-password  -uroot -p${SQL_PASSWORD} &>> $LOGFILE 
    stat $? 
fi 
